...
> Define your types once in Rust. Get perfectly matching types in TypeScript, Python, Go, Swift, Kotlin, and GraphQL — automatically, forever.
...
|:---:|:---:|:---:|:---:|
| proc macro powered | 6+ languages | forever in sync | zero boilerplate |
...
### ✅ Core Generation Platform (v0.4.0)
...
| Rust Type | TypeScript | Python | Go | Swift | Kotlin | GraphQL |
|---|---|---|---|---|---|---|
| `String` | `string` | `str` | `string` | `String` | `String` | `String` |
| `u8`–`u32`, `i8`–`i32`, `f32`, `f64` | `number` | `int` / `float` | `uint*` / `int*` / `float*` | `UInt*` / `Int*` / `Float`/`Double` | `UInt` / `Int` / `Float`/`Double` | `Int` / `Float` |
| `u64`, `i64` | `bigint` | `int` | `uint64` / `int64` | `UInt64` / `Int64` | `ULong` / `Long` | `String` |
| `bool` | `boolean` | `bool` | `bool` | `Bool` | `Boolean` | `Boolean` |
| `Option<T>` | `T \| undefined` | `Optional[T]` | `*T` with `omitempty` | `T?` | `T? = null` | nullable (no `!`) |
| `Vec<T>` | `T[]` | `list[T]` | `[]T` | `[T]` | `List<T>` | `[T!]` |
| `HashMap<K,V>` | `Record<K, V>` | `dict[K, V]` | `map[K]V` | `[K: V]` | `Map<K, V>` | `JSON` |
| `Uuid` | `string` | `UUID` | `string` | `UUID` | `String` | `ID` |
| `DateTime<Utc>` | `string` | `datetime` | `time.Time` | `Date` | `kotlinx.datetime.Instant` | `DateTime` |
